"use strict";

/******************************************************************************
 * Handling navbar clicks and updating navbar
 */


/** Show main list of all stories when click site name */
function navAllStories(evt) {
  console.debug("navAllStories", evt);
  hidePageComponents();
  putStoriesOnPage();
}
//Event listener navAllStories
$body.on("click", "#nav-all", navAllStories);


/** Show login/signup on click on "login" */
function navLoginClick(evt) {
  console.debug("navLoginClick", evt);
  hidePageComponents();
  $loginForm.show();
  $signupForm.show();
}
//Event listener navLoginClick
$navLogin.on("click", navLoginClick);


/** When the user clicks submit a form */
function navSubmitStory(evt) {
  console.debug("navSubmitStory", evt);
  hidePageComponents();
  $submitStoryForm.show();
}
//Event listener navSubmitStory
$navSubmit.on("click", navSubmitStory);


/** Show users stories */
function navShowMyStories(evt) {
  console.debug("navShowMyStories", evt);
  hidePageComponents();
  putMyStoriesOnPage();
}
//Event listener navShowMyStories
$navMyStories.on("click", navShowMyStories);


/** Show favorites */
function navShowFavorites(evt) {
  console.debug("navShowFavorites", evt);
  hidePageComponents();
  putFavoritesOnPage();
}
//Event listener navShowFavorites
$navFavorites.on("click", navShowFavorites);


/** When a user first logins in, update the navbar to reflect that. */
function updateNavOnLogin() {
  console.debug("updateNavOnLogin");
  $navUserLinks .show();
  $navLogin.hide();
  $navLogOut.show();
  $navUserProfile.text(`${currentUser.username}`).show();
}


/** When a user logs out. */
function updateNavOnLogout() {
  console.debug("updateNavOnLogin");
  $navUserLinks.hide();
  $navLogin.show();
  $navLogOut.hide();
}
